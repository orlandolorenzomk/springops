...
if ! git clone --verbose --single-branch -b "$BRANCH" "$GIT_URL" "$CLONE_DIR" 2>&1 | tee -a /tmp/script_output.log; then
...
if [ "$DEPLOY_TYPE" = "CLASSIC" ]; then
  ...
  if ! git checkout -b "$DEPLOY_BRANCH" 2>&1 | tee -a /tmp/script_output.log; then
    fail 1 "Failed to create deploy branch $DEPLOY_BRANCH" "$(cat /tmp/script_output.log)"
  fi

  if ! git push origin "$DEPLOY_BRANCH" 2>&1 | tee -a /tmp/script_output.log; then
  ...
  if ! git checkout "$DEPLOY_BRANCH" 2>&1 | tee -a /tmp/script_output.log; then
